...
`ng update` upgrades the packages, rewrites `package.json`, and then runs that version's migrations to keep your code compiling and idiomatic. You normally do **not** invoke these by hand — `ng-migrate`'s job is the `ng generate` modernization schematics; this file is reference for when a version bump is what's actually wanted.
...
The clean-tree precondition and the **gauntlet** from the main workflow apply here too — start clean, then build/lint/test the upgraded tree before committing.
...
This table will differ on every Angular version — regenerate it from `migrations.json` for the version you're on.
